...
#include "eufs_msgs/msg/cone_array_with_covariance.hpp"
#include "kvaser_reader_driver/msg/can_data.hpp"
#include "std_srvs/srv/set_bool.hpp" // Include service header
...
public:
    StartFinishDetectionNode()
        ...
        // Initialize service for start/finish control
        control_service_ = this->create_service<std_srvs::srv::SetBool>(
            "start_finish_control",
            std::bind(&StartFinishDetectionNode::handleControlCommand, this, std::placeholders::_1, std::placeholders::_2));
    }

private:
    void processCones(const eufs_msgs::msg::ConeArrayWithCovariance::SharedPtr msg) {
        if (!tracking_started_) {
            return; // Ignore cones if not tracking
        }
        ...
        // If we are tracking, check if we've exited the radius
        double exit_distance = calculateDistanceBetweenPoints(closest_middle_point, initial_middle_point_);
        if (exit_distance > EXIT_RADIUS || msg->big_orange_cones.empty()) {
            exited_radius_ = true;
        }

        // Trigger finish if we have exited the radius and returned
        if (exited_radius_ && current_distance < START_RADIUS && !msg->big_orange_cones.empty()) {
            publishFinishMessage();
            tracking_started_ = false; // Reset tracking
            exited_radius_ = false;
    ...
    void handleControlCommand(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
        ...
        if (request->data) {
            ...
            RCLCPP_INFO(this->get_logger(), "Tracking stopped.");
        }
    }
    ...
    rclcpp::Publisher<kvaser_reader_driver::msg::CanData>::SharedPtr finish_publisher_;
    rclcpp::Subscription<eufs_msgs::msg::ConeArrayWithCovariance>::SharedPtr cone_subscriber_;
    rclcpp::Service<std_srvs::srv::SetBool>::SharedPtr control_service_; // Service for start/finish control
